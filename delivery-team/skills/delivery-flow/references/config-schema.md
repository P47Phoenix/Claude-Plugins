...
|-----|------|----------|---------|-------------|-----------|-------------|
| `config_version` | string | yes | "2.9" | semver string | auto | delivery-flow (migration check) |
| `prose_style` | string | no | caveman-lite | caveman-lite, standard | defaults | delivery-flow (Phase 0 read; Phase 4 Step 4/5/7 conditional PROSE STYLE block injection per ADR-tk3-001) |
| `routing.force_type` | string/null | no | null | GREENFIELD, FEATURE, BUG_FIX, DESIGN, GAME_DEV+GREENFIELD, GAME_DEV+FEATURE, GAME_DEV+BUG_FIX, SPIKE, DOCS_ONLY, or null | defaults | delivery-flow (optional opt-in override for Phase 1 detection — Phase 1 still runs and is logged, but routing uses the pin) |
| `pipeline.enforce_self_write_block` | boolean | no | true (fresh v2.7), false (tolerantly-parsed v2.6) | true/false | defaults | enforce_pipeline_scope hook (activate soft-deny on orchestrator-origin writes under `.delivery/artifacts/**`) |
...
| `tech_stack.clean_code_guide` | string | no | "" | file path or empty string (empty = use built-in) | defaults | developer, godot, delivery-flow (clean code reference selection) |
| `tech_stack.clean_code_enforcement` | string | no | "block" | block, warn | defaults | developer, godot, delivery-flow (clean code enforcement level) |
...
| `personas.categories` | list[string] | no | auto (from runtime-detected type) | gamers, web-users, enterprise, demographics | Q10 | user-feedback (library selection) |
...
| `presentation.default_format` | string | no | "structured-markdown" | structured-markdown, marp, paste-ready, pptx | defaults | presentation (output format) |
| `presentation.default_audience` | string | no | "technical" | technical, executive, investor, client-facing, casual | defaults | presentation (audience mode) |
| `presentation.speaker_notes` | boolean | no | false | true/false | defaults | presentation (include speaker notes) |
| `presentation.save_to_artifacts` | boolean | no | true | true/false | defaults | presentation (save to .delivery/artifacts/presentations/) |
| `presentation.marp_theme` | string | no | "default" | default, gaia, uncover, custom path | defaults | presentation (Marp theme) |
| `presentation.staleness_warning_days` | integer | no | 7 | 1-30 | defaults | presentation (stale artifact warning) |
| `presentation.vocabulary_overrides` | map | no | {} | term: replacement pairs | defaults | presentation (custom jargon translations) |
| `presentation.pptx_template` | string | no | "" | file path to .pptx template (empty = blank presentation) | defaults | presentation (PPTX template for branding) |
| `presentation.pptx_font` | string | no | "Calibri" | font family name | defaults | presentation (PPTX font override) |
| `presentation.pptx_accent_color` | string | no | "#2d5aa0" | hex color string (#RRGGBB) | defaults | presentation (PPTX accent color override) |
| `presentation.narrative.emphasis` | boolean | no | true | true/false | defaults | presentation (enable/disable emphasis selection editorial pass) |
| `presentation.narrative.cutting` | boolean | no | true | true/false | defaults | presentation (enable/disable information cutting editorial pass) |
| `presentation.narrative.framing` | boolean | no | true | true/false | defaults | presentation (enable/disable audience-specific framing editorial pass) |
| `presentation.narrative.tension` | boolean | no | true | true/false | defaults | presentation (enable/disable narrative tension editorial pass) |
| `presentation.light_mode` | string | no | "auto" | auto, always, never | defaults | presentation (light mode activation strategy) |
| `presentation.thresholds` | map[string, integer] | no | {} | type-name: seconds pairs (e.g., sprint-review: 120). 0 = unlimited. | defaults | presentation (per-type threshold overrides) |
| `presentation.thresholds_default` | integer | no | 90 | 0-600 (0 = unlimited) | defaults | presentation (global threshold override in seconds) |
...
| Key | GREENFIELD | FEATURE | BUG_FIX | DESIGN | GAME_DEV+ | SPIKE | DOCS_ONLY |
|-----|-----------|---------|---------|--------|-----------|-------|-----------|
| `pipeline.checkpoints` | all 4 | [refine, uat] | [uat] | [refine, architect] | all 4 | [] | [uat] |
| `personas.categories` | [web-users] | [web-users] | [] | [] | [gamers] | [] | [] |
| `pipeline.collaboration_patterns` | all 6 | all 6 | [evaluator-optimizer, decision-routing] | all 6 | all 6 | [evaluator-optimizer] | [evaluator-optimizer] |
...
```yaml
config_version: "2.9"
prose_style: caveman-lite  # caveman-lite (default) | standard — narrative-prose discipline for agent dispatches and DoD verdict prose. See ADR-tk3-001.
routing:
...
| Key | Deprecated In | Status | Migration |
|-----|---------------|--------|-----------|
| `project_type` | 2.7 | **Warn-and-drop** — tolerantly parsed from v2.6 configs, logged as deprecated, ignored for routing. | If you previously pinned `project_type: X`, set `routing.force_type: X` instead. Phase 1 detection now runs on every pipeline invocation regardless. |

**Migration note (v2.6 → v2.7)**: The `project_type` key is removed from active
config. Phase 1 detection runs on every invocation. When the loader encounters a
v2.6 config with a bare `project_type`, it tolerantly drops the key, emits a
one-line deprecation warning to both the stage banner and `.delivery/state.md`
run log, and proceeds. If the user pinned the type intentionally, they must
migrate to `routing.force_type` within one run to restore the intended behavior.
See ADR-002 for the full rationale.

**Migration note (v2.7)**: `pipeline.enforce_self_write_block` is new. Fresh
v2.7 configs default to `true`. Tolerantly-parsed v2.6 configs default to
`false` (forward-looking activation, resolves the dogfood paradox — see ADR-001).

**Migration note (v2.8 → v2.9)**: Existing v2.8 (and earlier) configs
auto-load `prose_style: caveman-lite` if the key is missing. To opt out, set
`prose_style: standard` at the top level of `.delivery/config.yml`. No
other migration is required; the orchestrator reads the key at Phase 0 and
conditionally injects the PROSE STYLE block at Phase 4 dispatch construction.
See ADR-tk3-001.
...
| 2.2 | 2026-03-25 | Added presentation.default_format, presentation.default_audience, presentation.speaker_notes, presentation.save_to_artifacts, presentation.marp_theme, presentation.staleness_warning_days, presentation.vocabulary_overrides for presentation skill configuration |
| 2.3 | 2026-03-27 | Added tech_stack.clean_code_guide (custom clean code reference file path, empty = use built-in), tech_stack.clean_code_enforcement (block/warn enforcement level for clean code violations) |
| 2.4 | 2026-04-04 | Added presentation.narrative.emphasis, presentation.narrative.cutting, presentation.narrative.framing, presentation.narrative.tension for per-pass narrative intelligence config toggles |
| 2.5 | 2026-04-04 | Added presentation.light_mode (auto/always/never), presentation.thresholds (per-type override map), presentation.thresholds_default (global threshold override) for light mode and graceful degradation |
| 2.6 | 2026-04-04 | Added presentation.pptx_template (template file path), presentation.pptx_font (font family override), presentation.pptx_accent_color (hex accent color override) for PPTX output branding. Updated presentation.default_format to accept "pptx" as a valid value. |
| 2.8 | 2026-04-05 | Added DESIGN as a valid project type for `routing.force_type`. DESIGN routes Idea/Refine/Design/Architect at full depth and skips Plan/Dev/UAT — for design-only engagements that produce a committed design package without implementation. |
| 2.9 | 2026-05-05 | Added top-level `prose_style: caveman-lite \| standard` key (default caveman-lite). Pipeline orchestrator reads at Phase 0 and conditionally injects PROSE STYLE block in agent dispatch templates per ADR-tk3-001. Auto-clarity exemptions apply (security/destructive/multi-step/clarification). DoD validator verdict-prose uses caveman-lite (STATUS verbatim, FINDINGS standard prose preserved). |
| 2.7 | 2026-04-05 | **Removed `project_type` from active schema** — Phase 1 detection now runs every invocation. Legacy key is warn-and-drop on tolerant parse. Added `routing.force_type` as optional opt-in override (Phase 1 still runs and is logged; routing uses the pin). Added `pipeline.enforce_self_write_block` (boolean, activates `enforce_pipeline_scope.py` soft-deny for orchestrator-origin writes to `.delivery/artifacts/**`; default true on fresh v2.7 configs, false on tolerantly-parsed v2.6 configs to resolve the dogfood paradox). See ADR-001, ADR-002, ADR-003. |
