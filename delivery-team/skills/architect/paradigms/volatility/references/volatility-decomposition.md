...
> **Rule:** Components are named after axes of volatility — not after deployment units, runtime platforms, or implementation technologies.
>
> **Forbidden vocabulary at decomposition time:** Lambda, ECR, ECS, SQS, DynamoDB, S3, EC2, Kubernetes, Docker, Python, Node, TypeScript, Go, Rust, Java, Express, FastAPI, Django, PostgreSQL, MySQL, MongoDB. These names belong in the Plan and Development stages — never in volatility decomposition.
>
> **Why this matters here:** The Golden Rule above guards against functional decomposition ("what the system does"). This sidebar guards against a subtler trap: **implementation-detail contamination** ("how the system runs"). A component called `LambdaInvoker` or `SQSQueueProcessor` is decomposed by infrastructure, not by volatility. Name it by the axis of change it encapsulates — `NotificationChannelAccessor`, `TaxEngine` — so the boundary survives a platform migration without renaming half the system.
