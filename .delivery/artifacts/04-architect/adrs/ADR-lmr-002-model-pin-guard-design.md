...
- **Scan-nothing is not clean.** Before the summary line the script prints `files-scanned <K>` (K = in-scope files actually read). The LAST stdout line stays `guard-scope hits <N> files <M>` (PRD format unchanged, so the canonical-command equivalence holds). The script exits 2 when `git ls-files` fails, when run outside a repository, or when K is 0. So an empty scope, a wrong cwd or a broken listing can no longer print the clean summary.
- **Guard canary AC.** In a fresh `mktemp -d` clone or worktree copy (never the operator's tree), write an untracked `.md` containing a synthetic pin from the fixture file, run `python3 scripts/check_model_pins.py --list`, and require: exit 1, the temp file in the listing with category `pin`, and `files-scanned` above 0. Then delete it and require the clean tree to exit 0 again. Runs in S1, in ship Block A (ADR-lmr-005) and in S7b.
