...
**D9. Guard hardening and AC additions (revision 4; QA W1, W2, W8; Security).** Design-level; the Plan writes the AC text.
- **File enumeration.** The script reads the scope with `subprocess.run(["git", "ls-files", "--cached", "--others", "--exclude-standard", "-z"], shell=False, check=True, capture_output=True)` and splits on NUL (a path with a newline or space cannot split). Symlinks are skipped (`os.path.islink` true, or not a regular file), so a link cannot pull an outside file into the scan. A tracked, in-scope regular file that cannot be opened or decoded FAILS CLOSED: exit 2 and a stderr line naming the path; it is never skipped as "no hits".
- **Scan-nothing is not clean.** Before the summary line the script prints `files-scanned <K>` (K = in-scope files actually read). The LAST stdout line stays `guard-scope hits <N> files <M>` (PRD format unchanged, so the canonical-command equivalence holds). The script exits 2 when `git ls-files` fails, when run outside a repository, or when K is 0 in the DEFAULT scope. So an empty scope, a wrong cwd or a broken listing can no longer print the clean summary. **Explicit `--paths` (revision 5, QA W2):** a strict run over an explicit `--paths` list that contains zero scannable files (for example a commit that touches only `.json` files, the pre-commit case) prints `files-scanned 0` and `guard-scope hits 0 files 0` and exits 0; that is the documented return code for that mode, so a legitimate `.json`-only commit is never blocked. Only the default-scope callers (ship steps 4 and 5, AC-3.1b, canary, S7b) require K above 0. A listing failure or unreadable in-scope file exits 2 in both modes.
- **Guard canary AC.** In a fresh `mktemp -d` clone or worktree copy (never the operator's tree), write an untracked `.md` containing a synthetic pin from the fixture file, run `python3 scripts/check_model_pins.py --list`, and require: exit 1, the temp file in the listing with category `pin`, and `files-scanned` above 0. Then delete it and require the clean tree to exit 0 again. Runs in S1, in ship Block A step 0 and in S7b (both re-run lists in ADR-lmr-005 item 8 name it explicitly, revision 5).
- **AC-3.1b caller rules.** The Plan's AC-3.1b runs the script with `--list`, asserts `returncode in (0, 1)` (any other code fails, so a crashed or renamed script cannot yield `n = 0`), asserts the last stdout line matches `^guard-scope hits [0-9]+ files [0-9]+$`, asserts `files-scanned` above 0, and compares the listing count `n` to the summary `hits`.
- **Hit-line prefixes are pinned (revision 5, DevOps).** `--list` prints one hit per line, each beginning with exactly `pin `, `stamp ` or `prose ` (category, one space). Ship step 5 counts them with `awk '/^(pin|stamp|prose) /{n++}'`, so a prefix change would silently zero the count. A fixture test (Plan AC) feeds one line of each category and asserts each hit line starts with its exact prefix and that the awk count equals the number of fixture hits.
- **AC-5.5c is a distinct-stream check.** Distinctness means the five streams have pairwise non-overlapping sets of `message.id` values (plus distinct hashes and distinct `session_id`), see ADR-lmr-004 section 5. A hand-edited `session_id` cannot fake disjoint message ids.
- **Known false-positive fixtures (QA W8).** `scripts/model_pin_fixtures.json` gains a `known_fp` list: `on 6.8 kernels`, `with 4.7 V rail`, `the 4.7 uF cap` (all three flag today) and `claude-plugins-v2`, each marked `expect: hit`. A later pattern change that flips one fails the fixture test, so the change is deliberate. AC-1.2a also gets a `must_pass` list of hardware-team-style prose that must NOT hit after the S1 whole-tree run (P5) decides them; entries land in the fixture file, not in this ADR.
- **AC-1.6b beyond the pin category.** The marker text `model-pin-ok` is tested against one line of EACH category: a `pin`-shaped line, a `stamp`-shaped line and a `prose`-shaped line, each carrying the marker; all three must still hit. A marker exemption added to the `stamp` or `prose` rule is then caught.
- **Workflow permissions.** `stale-model-id-guard.yml` (D7) declares `permissions: contents: read` at workflow level and uses `actions/checkout` with `persist-credentials: false`; the job has no secrets and no write scope. Same for the `push` trigger added to `skill-line-budget.yml` (ADR-lmr-005 item 8). Temp paths in every local gate and AC use `mktemp`.
- **Hooks use NUL delimiters.** Both hooks pass file lists with `-z` and `xargs -0` (the D8 pre-commit reference already does; the pre-push hook follows).
