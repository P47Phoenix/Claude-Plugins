...
**Revision**: 5 (DoD round-3 self-correction: QA B2 plus warnings; see the Revision 5 changelog at the end of this file). Revision 4 = DoD round 2 (QA B1 plus should-fix findings). Revision 3 = adversarial loop 3 (see the Revision 3 changelog and the `cap_reached` exit record).
...
Producer files: `lib/metrics.py`, `lib/runner.py`, `lib/report.py`, `lib/aggregator.py` (pass-through only), `lib/baseline.py`, `run_smoke.py`. Validator files: `tests/test_model_capture.py`, `tests/fixtures/stream_real_shape.jsonl` and `.provenance.txt`. Interface, ordering (stub, red validator, fix), schema 2, budget-stop mapping and the init-baseline rule are in ADR-lmr-004. The fixture's `real_shape` failure message names the defect it catches. **P0 stub rule (revision 4, QA B1; ADR-lmr-004 section 6 items 1 and 2).** The P0 stub keeps every pre-existing function body unchanged (AST check) with ONE named exemption, `report.py::build_report`, which gains the pinned keys (now including `model_pin_env`) and the nested `tokens.cache_hit_ratio`, constants only (the gate strips those entries, the nested one from the `tokens` dict literal, and requires the rest to equal the `base_sha` body, so a computed key or any other edit fails). `Metrics` gains three constant-default fields `model_primary`, `models_observed`, `cache_hit_ratio` at P0 (gate rule 1d), so tests that read them fail by `AssertionError`, not `AttributeError` (revision 5, ADR-lmr-004 section 6 items 1b to 1d). New stub functions are inert by AST (only `pass`, docstring, or return of a constant) and never raise (revision 5). Red-first at `validator_start` means the `real_shape` and capture-failure tests are collected, FAIL, and each failure message begins `AssertionError` or `Failed: DID NOT RAISE`; `ImportError`, `ModuleNotFoundError`, `SyntaxError`, `TypeError`, `AttributeError`, `NameError`, `NotImplementedError` or a collection error voids the evidence. Also revision 4: `_spawn_and_tee` (runner.py:167, the only caller of `_build_claude_command` at :175) joins the interface table; `run_smoke.py` gets `--model` (opus only) and `--effort`; `sample-<n>.jsonl` is the full scrubbed stream (needed for the `message.id` distinctness rule); committed captures pass a secret and path scrub (ADR-lmr-004 sections 5, 8, 9). Revision 3: the pinned `build_report` keys and the `init_baseline` inputs are in ADR-lmr-004 section 2 (F8); red-first is also measured at `validator_start` (F6); an outcome-failed sample aborts `--init-baseline` (F9); every S5a and S5b dispatch passes `--out-dir "$SMOKE_OUT"` (`$HOME/.cache/lmr-smoke-out/run-2026-05-28-o48m`, durable, revision 5) so run output never lands untracked in the tree (F2).
...
- Revision 3 handoff: S1 and S7 record `git config core.hooksPath` in the log and run `git config core.hooksPath .githooks` first, or state the hooks are inert (F14); S5a and S5b dispatch prompts pass `--out-dir "$SMOKE_OUT"` (`$HOME/.cache/lmr-smoke-out/run-2026-05-28-o48m`, durable, revision 5) (F2); S2 dispatches never commit, the orchestrator commits serially with pathspecs (F12); S3 writes both ledgers and staggers `fitness_review_due` (F7, F15); the manifest set is per ADR-lmr-005 item 4b, with the recovered form for pre-writer Stage 4 rounds (F4, F5).
...
| F2 smoke run output untracked in tree | significant | FIXED. `--out-dir "$SMOKE_OUT"` (`$HOME/.cache/lmr-smoke-out/run-2026-05-28-o48m`, durable, revision 5) in every S5 dispatch; S7 handoff. | ADR-lmr-004 section 7; sections 4 (S5), 12 |
...
### ADR status after revision 4 (superseded by the note after the Revision 5 changelog)
...
## Revision 5 changelog (DoD round 3: `dod/qa-review.md` round 2, NOT_DONE, blocking B2; devops round-2 warnings)

| Finding | Disposition | Location |
|---|---|---|
| QA B2.1 rule 1b(ii) names a non-existent `metrics["tokens.cache_hit_ratio"]` target | FIXED. Strip rule restated against the real structure: the constant `cache_hit_ratio` entry inside the nested `tokens` dict literal. Constant-literal requirement kept. | ADR-lmr-004 section 6 item 1b, section 2 `build_report` row |
| QA B2.3 `model_pin_env` missing from pinned set | FIXED. Added to the set and to the section 2 key list. | ADR-lmr-004 section 6 item 1b |
| QA B2.2 new `Metrics` fields fail with `AttributeError` at P0 | FIXED. Chosen: constant-default `Metrics` field additions allowed at P0, gate rule 1d (class-body gate). Rejected: "tests use getattr" (prose, unchecked). | ADR-lmr-004 section 1 item 6, section 6 item 1d |
| QA W1 inert stubs not machine-checked | FIXED. Rule 1c is an AST check (`pass`, docstring, return constant only). `load_baseline` stub now returns `{}` (was `json.load`, a call). | ADR-lmr-004 section 6 item 1c |
| QA W2 canary missing from Block A step 0 and S7b lists; D9 exit 2 blocks `.json`-only `--paths` runs | FIXED. Canary named in both lists. Exit 2 for K == 0 applies to DEFAULT scope only; explicit `--paths` with zero scannable files exits 0 with `files-scanned 0` (documented rc). | ADR-lmr-005 item 8; ADR-lmr-002 D9 |
| QA W3 AC-5.5c missing in Plan-carry; `message.id` presence | FIXED. In Plan-carry; presence asserted in the fixture test; fallback `uuid`, else WEAKENED report, never silent. | ADR-lmr-004 section 5; Plan-carry below |
| QA W4 transcript layout observed on one CLI version | FIXED. Checker tolerates drift, prints `TRANSCRIPT_CHECK=SKIPPED layout-drift <id>`, never claims `RAN`. Plan checker must flag drift. | ADR-lmr-005 item 8 (S7b transcript check) |
| QA W5 step 9 spend-check order; S5b reports only in `${TMPDIR:-/tmp}`; F2 out-dir statement | FIXED. Order: spend check before each S5b run and at Block B step 1a; step 9 is operator go then push. Reports in durable `$SMOKE_OUT=$HOME/.cache/lmr-smoke-out/run-2026-05-28-o48m`, plus tracked `07-uat/s5b-spend.txt`. F2 statement updated everywhere it appeared (sections 4 and 12 and Revision 3 changelog now say `$SMOKE_OUT`). | ADR-lmr-004 sections 7, 8; ADR-lmr-005 item 8 step 9 |
| QA W6 | No action (as instructed). | |
| DevOps hit-line prefixes | FIXED (design), Plan writes fixture AC. | ADR-lmr-002 D9 |
| DevOps `spent` extraction, missing total counts as failure | FIXED. Snippet: missing `report.json` = $3.00, non-numeric `cost_usd` = `BAD_COST` exit 2. | ADR-lmr-004 section 8 |

**Experiment evidence** (`/home/meconnelly/.claude/jobs/af2deffc/tmp/exp`, real `lib/` copy, `gate2.py`, `mk2.py`, `red2.py`). Gate results: correct P0 PASS; computed `session_id` FAIL; computed nested `cache_hit_ratio` FAIL; edited `cost_usd` FAIL; `metrics[...] =` assign FAIL; real impl in new function FAIL (`not inert`); existing function body edit FAIL; method added to `Metrics` FAIL; non-constant `Metrics` default FAIL; hardcoded `model_resolved` constant PASSES the gate (by design). Red run on the correct P0 with direct attribute reads: rc 1, six failures, all permitted (`AssertionError` x5, `Failed: DID NOT RAISE` x1), zero voiding types. Hardcoded-constant P0: the value test passes, so the "every test FAILED" rule catches it (5 failed, 1 passed). Not run: the gate is experiment code, not the shipped checker; the Plan writes the real one.

**PRD conflicts, stated not changed.** (1) The PRD section 5 table and FR-5.1 say `tokens.cache_hit_ratio` "in `metrics`"; the real report has it under `report["tokens"]`, and the design keeps that (no PRD text edited). (2) The PRD AC-5.5c snippet lacks the `message.id` presence check and fallback; ADR text says what the Plan adds.

### Plan-carry additions (revision 5)

5. AC-5.5c distinctness (non-overlapping `message.id` sets, distinct hash and `session_id`, WEAKENED reporting when ids are absent) is in Plan-carry item 3 scope.
6. Fixture-test AC pinning the `--list` hit-line prefixes `pin `, `stamp `, `prose ` and the awk count (ADR-lmr-002 D9).
7. AST P0 gate rules 1a to 1d (ADR-lmr-004 section 6), including the `Metrics` field gate and the inert-function check; the Plan checker must flag transcript layout drift (ADR-lmr-005).
8. Spend snippet AC: a report with missing or non-numeric `cost_usd` fails, a missing `report.json` counts $3.00.

### ADR status after revision 5

Left PROPOSED. The orchestrator flips all five after QA re-validates B2.
