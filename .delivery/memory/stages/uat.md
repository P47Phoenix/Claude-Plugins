...
**Entries**: 5 | **Last updated**: 2026-04-01
...
- LLM card/domain knowledge is unreliable for correctness gates. Rules Judge missed Sejiri Refuge (W/U) in a W/B deck because it relied on training data. Programmatic API validation (validate-deck command) is mandatory — never trust LLM memory for format legality. (validated: 1, last: run-2026-04-02-k3r9)
- Single-source pricing creates false confidence. TCGPlayer showed $97 for a deck that costs $150+ on Card Kingdom. Dual-vendor pricing (via Archidekt API) exposes real budget gaps. Always show multiple vendor prices. (validated: 1, last: run-2026-04-02-k3r9)
