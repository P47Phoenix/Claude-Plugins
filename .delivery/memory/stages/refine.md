...
- **DoD gate criteria for PRD validation MUST be explicit about TARGET state vs CURRENT state.** PRD validation answers "is the AC well-formed and runnable?" — NOT "does the AC pass today?" (it shouldn't; the work hasn't been done). Stage 6 Dev validation answers "does the AC pass?" — the implementation either satisfies it or doesn't. Conflating the two wastes a DoD round (Wave 1 Stage 2 R1 cost a re-dispatch when Dev validator treated FRs describing W1-6 deliverables as if they were prerequisites). Action: validator prompts MUST state the target state framing explicitly; e.g., "Verify the AC command is parseable (well-formed). Do NOT verify whether it passes today — that's Stage 6's gate." (validated: 1, last: run-2026-05-04-tk1)
