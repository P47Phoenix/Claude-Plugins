...
- **FEATURE-execution-of-pre-planned-waves with binding-decisions-in-memory pattern**: when a multi-wave initiative pre-loads its conflict rulings + per-skill model map + tier values into a memory topic file (e.g., `topics/skill-token-economy.md`), the FEATURE pipeline runs WITHOUT re-debating decisions during stage execution. At run-2026-05-03-tk0e Wave 0, 4 of 7 stages passed first-try DoD; at run-2026-05-04-tk1 Wave 1, similar 5/9 stage-stories first-try with 2 real defects caught and 1 false-positive corrected. Action: for any multi-wave initiative, author a `topics/<initiative>.md` memory file BEFORE invoking delivery-flow; treat it as binding context. (validated: 2, last: run-2026-05-04-tk1)

- **Story consolidation by file scope (not by WI count) is the right pattern for FEATURE-execution-of-pre-planned-waves.** When a wave has N mechanically-independent WIs and M < N file-scope groups (i.e., multiple WIs touch the same file), collapse into M stories rather than dispatching N separate developer implementations. Wave 1 (run-2026-05-04-tk1) had 7 WIs collapsed into 3 file-scope stories: Story 1 (delivery-flow/SKILL.md edits, covering W1-1+W1-2+W1-5-doc+W1-6); Story 2 (frontmatter rollout across 12 SKILL.md, covering W1-3+W1-4+W1-7); Story 3 (audit_agent_prompt.py extension, W1-5-hook). Stage 6 dispatched 3 implementations in parallel + 12 DoD validators, vs. naive 7 implementations + 28 validators. Same DoD coverage, ~50% fewer Agent dispatches. (validated: 1, last: run-2026-05-04-tk1)
