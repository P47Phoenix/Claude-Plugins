...
- **Pipeline artifacts live at `.delivery/artifacts/<NN>-<stage>/`** — NOT under `<plugin>/references/` or other plugin-internal paths. ADRs go in `.delivery/artifacts/04-architect/adrs/`. Implementation reports in `.delivery/artifacts/06-dev/developer/`. Dogfood evidence in `.delivery/artifacts/06-dev/dogfood-evidence/`. DoD reviews in `.delivery/artifacts/<NN>-<stage>/dod/`. Validator prompts MUST cite canonical paths or risk false-positive "phantom path" findings. Wave 1 Story 1 + Wave 2 Story 1 BOTH had validators look in wrong dirs (canonical extracted-references path = `<plugin>/references/` or `delivery-team/references/shared/`); both wasted a re-dispatch. Future validator prompts MUST inline the exact canonical path expected. (validated: 2, last: run-2026-05-05-tk2)

## Validator Defect Classes (run-2026-05-05-tk2)

- **Validator semantic-understanding gaps recur** — validators repeatedly treat downstream-stage deliverables as prerequisites for the current stage. Wave 1 Stage 2: Dev validator treated W1-6 deliverables (model frontmatter) as Stage 2 PRD prerequisites. Wave 2 Stage 4: Dev validator treated W2-0 Stage 6 admin deliverable (skill-budgets.json updates) as Architect-stage prerequisite. Wave 2 Story 5: QA validator interpreted "developer file removed from known_debt" as a defect rather than the success state. Action: validator prompts MUST explicitly state validation target ("validate the SPEC/PRD/ADR" vs "validate the IMPLEMENTATION") + explicit examples of what counts as PASS. (validated: 3, last: run-2026-05-05-tk2)

- **Registry consistency is multi-source** — when known-debt or tier classification lives in BOTH a JSON file AND a Python script (e.g., governance/skill-budgets.json + scripts/check_skill_budgets.py), edits MUST update BOTH or downstream validators flag inconsistency. Wave 2 Story 5 needed 4 rounds of QA validation (architect tier mismatch JSON vs frontmatter, JSON vs Python list). Action: Wave 3 should add a CI lint that diff-checks JSON ↔ Python KNOWN_DEBT for consistency. (validated: 1, last: run-2026-05-05-tk2)

- **DoD STATUS-line format diversity breaks literal-grep checklists** — Wave 0 + Wave 2 release-plans both used `grep "STATUS: DONE"` to verify DoD review files; both flagged because validators emit diverse formats: `STATUS: DONE`, `Status: PASS`, `Gate Status: DONE`, `status: PASS` (lowercase), inline frontmatter status fields. Action: standardize on `grep -iE 'status:?\s*(done|pass)'` in checklists, OR enforce a single format in validator prompt templates. Same defect family as "PRD §7 stale-ID regexes must exempt provenance comments" lesson. (validated: 2, last: run-2026-05-05-tk2)
